            ...
            {loadingWeight ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={weightData} margin={{ top: 20, right: 20, left: 0, bottom: 20 }}>
                  <XAxis dataKey="date" tickFormatter={(v) => new Date(v).toLocaleDateString(undefined, {month:'short', day:'numeric'})} stroke="#444" fontSize={12} fontFamily="monospace" tickMargin={15} />
                  <YAxis stroke="#444" fontSize={12} fontFamily="monospace" tickMargin={10} />
            ...
            {loadingDexa ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={[...dexaScans].reverse()} margin={{ top: 20, right: 20, left: 0, bottom: 20 }}>
                  <XAxis dataKey="scanDate" tickFormatter={(v) => new Date(v).toLocaleDateString(undefined, {month:'short', day:'numeric'})} stroke="#444" fontSize={12} fontFamily="monospace" tickMargin={15} />
                  <YAxis yAxisId="left" stroke="#444" fontSize={12} fontFamily="monospace" domain={['dataMin - 2', 'dataMax + 2']} tickMargin={10} />
                  <YAxis yAxisId="right" orientation="right" stroke="#444" fontSize={12} fontFamily="monospace" domain={['dataMin - 5', 'dataMax + 5']} tickMargin={10} />
                  <Tooltip contentStyle={{ backgroundColor: 'hsl(var(--card))', borderColor: 'hsl(var(--border))', fontFamily: 'monospace', borderRadius: '8px' }} />
                  <Area yAxisId="left" type="monotone" dataKey="bodyFatPercent" name="Body Fat %" stroke="#3b82f6" strokeWidth={3} fill="#3b82f6" fillOpacity={0.15} />
                  <Area yAxisId="right" type="monotone" dataKey="leanMassLbs" name="Lean Mass (Lbs)" stroke="#10b981" strokeWidth={3} fill="#10b981" fillOpacity={0.15} />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
